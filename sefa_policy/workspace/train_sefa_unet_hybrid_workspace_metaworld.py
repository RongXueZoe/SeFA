...
from sefa_policy.policy.sefa_unet_hybrid_image_metaworld_policy import DiffusionUnetHybridImageSeFAMetaworldPolicy
...
from sefa_policy.dataset.mt10_sampler import train_sampler, test_sampler, Normalizer

from scripts.find_nearest import find_nearest_action_sequence, load_all_samples

OmegaConf.register_new_resolver("eval", eval, replace=True)

class TrainSeFAUnetHybridWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: DiffusionUnetHybridImageSeFAMetaworldPolicy = hydra.utils.instantiate(cfg.policy)

        self.ema_model: DiffusionUnetHybridImageSeFAMetaworldPolicy = None
    ...
    def run(self):
        ...
        lr_scheduler = get_scheduler(
            ...
            num_training_steps=(train_sampler.max_episode_length * cfg.training.num_epochs)
        ...
        print('env_runner loaded')
        if cfg.use_wandb:
            # configure logging
            wandb_run = wandb.init(
                dir=str(self.output_dir),
                config=OmegaConf.to_container(cfg, resolve=True),
                # id="<run ID>", resume="<resume>",
                **cfg.logging
            )
            wandb.config.update(
                {
        ...
        if cfg.task.load_from_pkl:
            ...
            dataset = hydra.utils.instantiate(cfg.task.train_dataset)
        ...
        print('normalizer loaded', normalizer)

        if cfg.mode == 'refine':
            print(f"Loading SeFA data from {cfg.sefa_name} to refine")
            rf2_path = f"data/mt50/{cfg.task.env_name}/{cfg.sefa_name}.pkl"
            action_sequences, target_poses, sampled_video_paths = load_all_samples(f"data/mt50/{cfg.task.env_name}")
            z0_cllt = {}
            z1_cllt = {}
            cond_cllt = {}
            with open(rf2_path, "rb") as f:
                ...
                for outer_idx in tqdm.tqdm(range(len(z1_cllt)), dynamic_ncols=True, leave=True):
                    for inner_idx in tqdm.tqdm(range(len(z1_cllt[outer_idx])), dynamic_ncols=True, leave=False):
                        rf2_action = torch.from_numpy(z1_cllt[outer_idx][inner_idx])    ## [16, 4]
                        rf2_cond = torch.from_numpy(cond_cllt[outer_idx][inner_idx])    ## [16, 4]
            ...
            with open(f"data/mt50/{cfg.task.env_name}/{cfg.sefa_name}_refined.pkl", "wb") as f:
        ...
        if cfg.mode == 'gen_data':
            ...
            gen_data = {"z0_cllt": z0_cllt, "z1_cllt": z1_cllt, "cond_cllt": cond_cllt}
            with open(f"data/mt50/{cfg.task.env_name}/{cfg.sefa_name}.pkl", "wb") as f:
                pickle.dump(gen_data, f)
            print(f"Generated RecFlow data saved to {f.name}")
            return
        ...
        with JsonLogger(log_path) as json_logger:
            ...
            for _ in range(cfg.training.num_epochs):
                ...
                if cfg.task.load_from_pkl:
                    ...
                    pbar = range(train_sampler.max_episode_length)
                    with tqdm.tqdm(pbar, desc=f"Training epoch {self.epoch}", 
                            leave=False, mininterval=cfg.training.tqdm_interval_sec, dynamic_ncols=True) as tepoch:
                        for batch_idx in enumerate(tepoch):
                            batch = train_sampler.obtain_samples(batch_idx, num_samples=cfg.dataloader.batch_size, env_update=None)
                            # device transfer
                            batch = dict_apply(batch, lambda x: x.to(device, non_blocking=True).float())
                            if train_sampling_batch is None:
                                train_sampling_batch = batch
                            ...
                            is_last_batch = (batch_idx == (train_sampler.max_episode_length-1))
                            if not is_last_batch and cfg.use_wandb:
                                # log of last step is combined with validation and rollout
                                wandb_run.log(step_log, step=self.global_step)
                                json_logger.log(step_log)
                ...
                if (self.epoch % cfg.training.val_every) == 0:
                    ...
                    if cfg.task.load_from_pkl:
                        ...
                        with torch.no_grad():
                            pbar = range(1)
                            with tqdm.tqdm(pbar, desc=f"Validation epoch {self.epoch}", 
                                    leave=False, mininterval=cfg.training.tqdm_interval_sec, dynamic_ncols=True) as tepoch:
                                for batch_idx in enumerate(tepoch):
                                    batch = test_sampler.obtain_exact_episodes(1)
                                    batch = dict_apply(batch, lambda x: x.to(device, non_blocking=True).float())
                                    loss = self.model.compute_loss(batch)
                                    val_losses.append(loss)
                                    if (cfg.training.max_val_steps is not None) \
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        batch = dict_apply(train_sampling_batch, lambda x: x.to(device, non_blocking=True).float())
                        agent_pos = batch["base_obs"]
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0:
                    ...
                    if cfg.checkpoint.topk.monitor_key in metric_dict.keys():
...
def main(cfg):
    workspace = TrainSeFAUnetHybridWorkspace(cfg)
    workspace.run()
