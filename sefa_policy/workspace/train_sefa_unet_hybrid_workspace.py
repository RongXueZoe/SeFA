...
from sefa_policy.policy.diffusion_unet_hybrid_image_policy import DiffusionUnetHybridImagePolicy
...
class TrainSeFAUnetHybridWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        if cfg.training.debug:
            cfg.use_wandb = False
            cfg.task.train_dataset.repeat_times = 1
        ...
        if cfg.mode == 'gen_data':
            ...
            with open(f"data/pushT/{cfg.task.env_name}/{cfg.sefa_name}_refined.pkl", "wb") as f:
        ...
        if cfg.mode == 'gen_data':
            ...
            if cfg.task.load_from_pkl:
                    with tqdm.tqdm(train_dataloader, desc=f"Generating SeFA data epoch={self.epoch}", 
                        ...
                        for batch_idx, batch in enumerate(tepoch):
                            ...
                            for nsample in range(cfg.sample_per_cond):
            ...
            gen_data = {"z0_cllt": z0_cllt, "z1_cllt": z1_cllt, "cond_cllt": cond_cllt}
            with open(f"data/pushT/{cfg.task.env_name}/{cfg.sefa_name}.pkl", "wb") as f:
                pickle.dump(gen_data, f)
            print(f"Generated RecFlow data saved to {f.name}")
            return
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        if (not is_last_batch) and (batch_idx != 0):
