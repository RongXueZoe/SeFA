...
import torch
import tqdm
from sefa_policy.env import AdroitEnv
from sefa_policy.gym_util.mjpc_diffusion_wrapper import MujocoPointcloudWrapperAdroit
from sefa_policy.gym_util.multistep_wrapper import MultiStepWrapper
from sefa_policy.gym_util.video_recording_wrapper import SimpleVideoRecordingWrapper
...
import sefa_policy.common.logger_util as logger_util
        ...
        def env_fn():
            return MultiStepWrapper(
                SimpleVideoRecordingWrapper(
                    # MujocoPointcloudWrapperAdroit(env=AdroitEnv(env_name=task_name, use_point_cloud=False),
        ...
        for episode_idx in tqdm.tqdm(range(self.eval_episodes), desc=f"Eval in Adroit {self.task_name} Pointcloud Env",
            ...
            while not done:
                ...
                with torch.no_grad():
                    ...
                    obs_dict_input['img'] = obs_dict['image'].unsqueeze(0).float()    ## [2, 3, 84, 84]
