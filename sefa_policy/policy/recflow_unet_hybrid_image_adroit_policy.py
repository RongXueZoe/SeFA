...
class SeFAUnetHybridImageAdroitPolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        self.use_rk45 = kwargs.get('use_rk45', False)
    ...
    def conditional_sample(self,
        ...
        with torch.no_grad():
            ...
            if self.mode == 'sefa':
                ...
                trajectory = x
            elif not self.use_rk45 and not self.mode == 'gen_data':
                x = trajectory
                self.sample_N = 100
                dt = 1. / self.sample_N
                ...
                for i in range(self.sample_N):
                    ...
                    num_t = i / self.sample_N * (self.sde.T - eps) + eps
                    vec_t = torch.ones(condition_data.shape[0], device=x.device) * num_t
                    drift = model(x, vec_t*self.noise_scheduler.config.num_train_timesteps, 
                        local_cond=local_cond, global_cond=global_cond)
    ...
    def compute_loss(self, batch):
        ...
        timesteps = t * self.noise_scheduler.config.num_train_timesteps
