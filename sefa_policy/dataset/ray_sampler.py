...
class RaySampler(Sampler):
    ...
    def start_worker(self):
        ...
        for worker_id in range(self._worker_factory.n_workers):
            self._all_workers[worker_id] = self._sampler_worker.remote(
                worker_id, self._envs[worker_id], self._agents[worker_id], self.batch_size, self.win_len, self._names[worker_id],
    ...
    def _update_workers(self, agent_update, env_update):
        ...
        for worker_id in range(self._worker_factory.n_workers):
            ...
            updating_workers.append(worker.update.remote(None, None))
        return updating_workers

    def obtain_samples(self, itr, num_samples, agent_update=None, env_update=None):
        ...
        while completed_samples < num_samples:
            ...
            for result in ready:
                ...
                num_returned_samples = len(episode_batch['imgs'])
                completed_samples += num_returned_samples
                batches.append(episode_batch)

        imgs = [np.stack(batch['imgs'], axis=0) for batch in batches]
        imgs = np.concatenate(imgs, axis=0)
        base_obs = [np.stack(batch["base_obs"], axis=0) for batch in batches]
        base_obs = np.concatenate(base_obs, axis=0)
        actions = [np.stack(batch['actions'], axis=0) for batch in batches]
        actions = np.concatenate(actions, axis=0)

        self.total_env_steps += actions.shape[0]
        return {
            "imgs": torch.from_numpy(imgs).float(),
            "base_obs": torch.from_numpy(base_obs).float(),
...
class SamplerWorker:
    ...
    def __init__(self, worker_id, env, agent, batch_size, win_len, name, worker_factory):
        # Must be called before pickle.loads below.
        self.inner_worker = worker_factory(batch_size=batch_size, win_len=win_len, worker_number=worker_id)
        self.worker_id = worker_id
        mt10 = metaworld.MT10()
        mt_custom_envs = mt10.train_classes
        env = mt_custom_envs[name]
        env = env()
        agent = policies[name]
